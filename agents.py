...
from core_systems import Genome, PrincipleSystem, SkillLibrary
from memory import (
    LongTermMemory,
    SelfModel,
    Episode,
...
class GlobalWorkspace:
    """
    The 'theater of consciousness' where different modules share information.
    Prioritizes messages based on salience.
    """
    def __init__(self, genome: Genome):
        self.genome = genome
        # Priority queue for workspace messages: (priority, timestamp, source, content)
        # Higher priority = lower number (standard heap behavior is min-heap, so we invert logic or use negatives)
        # Implementation: We will use negative priority so standard heapq pops highest priority first.
        self.message_queue = [] 
        self.history = deque(maxlen=100)
        self.active_content: Optional[Dict[str, Any]] = None
        self.msg_counter = 0 # Add a counter to break ties
        self.working_memory = WorkingMemory()

    def post(self, priority: float, source: str, content: Any) -> None:
        ...
        self.history.append({"time": timestamp, "source": source, "content": content, "p": priority})
        
        # High priority items immediately become active content
        if priority > 8.0:
            self.active_content = {"source": source, "content": content}
            log("WORKSPACE", f"📢 Interrupt: [{source}] {str(content)[:50]}...", Term.ORANGE)

    def get_most_salient(self) -> Optional[Tuple[float, str, Any]]:
        """Retrieve the most important message"""
...
# agents.py - Update PlanningAgent
        ...
        try:
            ...
            if action.action_type == ActionType.PROCESS_INPUT:
                ...
                if success and next_action:
                    # If the bridge returned a continuation, we queue it immediately
                    # This keeps the "loop" alive in the Executive's queue
                    self.executive.add_goal(next_action, priority=goal.priority + 0.1)
                    return True, None
                elif success:
                    return True, None
                else:
                    return False, FailureType.LACK_OF_KNOWLEDGE
